...
static struct nh_info NickelHome = (struct nh_info){
    ...
    .uninstall_flag  = HM_CONFIG_DIR "/uninstall",
#ifdef HM_UNINSTALL_CONFIGDIR
    .uninstall_xflag = HM_CONFIG_DIR,
#else
...
static struct nh_hook NickelHomeHook[] = {
    ...
    {.sym = "_ZN12HomePageViewC1EP7QWidget", .sym_new = "_nh_homepageview_hook", .lib = "libnickel.so.1.0.0", .out = nh_symoutptr(HomePageView_HomePageView), .desc = "home page widget hiding (15505+)", .optional = true}, //libnickel 4.23.15505 * _ZN12HomePageViewC1EP7QWidget
...
static int hm_init() {
#ifdef HM_UNINSTALL_CONFIGDIR
    HM_LOG("feature: HM_UNINSTALL_CONFIGDIR: true");
#else
    HM_LOG("feature: HM_UNINSTALL_CONFIGDIR: false");
#endif

    // parse (and cache) the config now so any errors are logged at startup
    hm_global_config_get("");

    return 0;
}

// hm_find_direct_child_widget returns the direct child QWidget of parent with
// the given object name, or NULL if there isn't one.
static QWidget *hm_find_direct_child_widget(QWidget *parent, const QString& objectName) {
...
// hm_find_home_widget resolves a home-screen widget under mainContainer by row
...
static QWidget *hm_find_home_widget(QWidget *root, const char *row_name, const char *widget_name) {
    ...
    for (QWidget *container : containers) {
        QWidget *row = hm_find_direct_child_widget(container, qRowName);
        if (!row)
            continue;
        if (!widget_name)
            return row;

        QWidget *widget = hm_find_direct_child_widget(row, qWidgetName);
        if (widget)
            return widget;
    }
...
// hm_hide_home_widget hides a home-screen widget. When keep_layout_space is
...
static void hm_hide_home_widget(QWidget *widget, bool keep_layout_space) {
    if (!keep_layout_space) {
        HM_LOG("hiding home widget '%s' by setting it invisible",
    ...
    if (widget->graphicsEffect() != effect) {
        HM_LOG("warning: could not attach opacity effect to home widget '%s'; visual-only hide may not work as expected",
            widget->objectName().isEmpty() ? "<unnamed>" : qPrintable(widget->objectName()));
    } else {
        HM_LOG("hiding home widget '%s' visually without collapsing layout space",
            widget->objectName().isEmpty() ? "<unnamed>" : qPrintable(widget->objectName()));
    }
...
extern "C" __attribute__((visibility("default"))) void _nh_homepageview_hook(HomePageView *_this, QWidget *parent) {
    HM_LOG("HomePageView::HomePageView(%p, %p)", _this, parent);
    ...
    for (size_t i = 0; i < sizeof(hide_rules) / sizeof(hide_rules[0]); i++) {
        const char *val = hm_global_config_get(hide_rules[i].config_key);
        if (!val || strcmp(val, "1"))
            continue;

        QWidget *w = hm_find_home_widget(_this, hide_rules[i].row_name, hide_rules[i].widget_name);

        if (w)
            hm_hide_home_widget(w, hide_rules[i].keep_layout_space);
        else if (hide_rules[i].widget_name)
            HM_LOG("warning: could not find home page widget '%s' under mainContainer.%s to hide (it may not exist on this firmware version)",
                hide_rules[i].widget_name,
                hide_rules[i].row_name);
        else
            HM_LOG("warning: could not find home page row '%s' under mainContainer to hide (it may not exist on this firmware version)",
                hide_rules[i].row_name);
    }
}
